...
AgentGit 的核心业务包，负责 Git checkpoint 工作流逻辑。

这个包不依赖 MCP SDK，不定义 MCP schema，也不输出 Agent Skill 文案。它只提供可复用的 TypeScript API，供 MCP、CLI、Skill fallback 或未来 SDK 入口调用。
